...
def save_predicted_depth(outputs:torch.Tensor,batch):
    depth_map=outputs["surf_depth"].squeeze(0)
    result=depth_map.detach().cpu().numpy()
    num=batch[0].idx.item()
    num_str=str(num).zfill(4)
    filename=num_str+".png.npy"
    np.save("data/jinguilou/predicted_depths/"+filename,result)

def get_pcd_from_plyfile(plyfile):
    ply=o3d.io.read_point_cloud(plyfile)
    points = np.asarray(ply.points)
    colors = np.asarray(ply.colors)
    return points,colors

def integrate_points(xyz1:Union[np.ndarray,torch.Tensor],
                     rgb1:Union[np.ndarray,torch.Tensor],
                     xyz2:Union[np.ndarray,torch.Tensor],
                     rgb2:Union[np.ndarray,torch.Tensor]):
