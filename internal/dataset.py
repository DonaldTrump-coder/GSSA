...
class Dataset(torch.utils.data.Dataset):
    ...
    def get_image(self, index) -> Tuple[str, torch.Tensor, Optional[torch.Tensor]]:#
        ...
        # TODO: resize
        pil_image = Image.open(self.image_set.image_paths[index])#PIL
        numpy_image = np.array(pil_image, dtype=np.uint8)#np
        ...
        if self.image_set.mask_paths[index] is not None:
            pil_image = Image.open(self.image_set.mask_paths[index])#mask
            ...
            mask = mask.permute(2, 0, 1).to(self.image_device)  # [channel, height, width]  CHW

        image = image.permute(2, 0, 1).to(self.image_device)  # [channel, height, width]  

        return self.image_set.image_names[index], image, mask#mask
    ...
    def setup(self, stage: str) -> None:
        ...
        if self.global_rank == 0 and stage == "fit":
            ...
            camera_to_world = torch.linalg.inv(
                torch.transpose(self.dataparser_outputs.train_set.cameras.world_to_camera, #world_to_camera
                                1, 
                                2)#
            ).numpy()
            cameras = []
